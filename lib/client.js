window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function cssFor(state) {
			...
			// The card's own toggle speaks last: an explicit direction for the card
			// alone, written after the mode's rule above (so it wins at equal
			// specificity) and as an author declaration (so it also wins over the
			// `dir` attribute the Auto pass writes on the card).
			if (cardDir !== null) rules.push(QUERY_SEL + ' { direction: ' + cardDir + '; text-align: start; }')
		...
		/**
		 * A direction chosen for the question card *alone*, from the toggle in its
		 * header: `null` follows the mode, `'rtl'` / `'ltr'` override it for the card
		 * and for nothing else — the transcript and the composer keep the mode they
		 * were given. It is deliberately not part of the stored preference: it is a
		 * correction applied to the question in front of you, not a setting.
		 */
		let cardDir = null
		...
		function commit(next) {
			...
			// elements the runtime passes govern — and what the card toggle reports.
			syncTableDirs()
			syncAutoPass()
			syncCardToggles()
		}
		...
		function setCardDir(card) {
			// The card's own toggle outranks the text: once the user has chosen a
			// direction for the card from its header, that is the direction it takes.
			const dir = cardDir !== null ? cardDir : firstStrong(card.textContent || '')
			if (dir === null) {
				clearAutoDir(card)
				return
			}
		...
		function syncAutoDirs() {
			...
			for (const card of document.querySelectorAll(QUERY_SEL)) {
				setCardDir(card)
				// With the toggle in charge the card carries one direction for
				// everything inside it, so the per-element marks are handed back rather
				// than left to disagree with the direction the user asked for.
				for (const el of card.querySelectorAll('[' + AUTO_DIR_MARK + ']')) clearAutoDir(el)
				if (cardDir !== null) continue
		...
		//#region question card toggle
		/**
		 * ## A direction button, in the card itself
		 *
		 * The card is the one surface where a direction can be wrong for a reason the
		 * mode cannot know: a reply in Arabic arrives, the mode is RTL, and this one
		 * question is in English. So the card carries a toggle of its own — a small
		 * button in its header that reports the direction the card has right now and
		 * swaps it for the other one on a click.
		 *
		 * It is deliberately card-only. Pressing it writes `cardDir`, which the
		 * stylesheet turns into one extra rule for `[data-question-key]` and nothing
		 * else: the transcript, the composer and every other card surface keep the
		 * mode they were given, and the settings row still shows that mode. The
		 * override is in memory only — it is a correction applied to the question in
		 * front of you, not a setting, and it does not survive a reload.
		 *
		 * The button is injected into the card's DOM rather than rendered by the GUI:
		 * the card belongs to another plugin, and its header actions are the only
		 * place a control can sit beside the minimise and close buttons. It is
		 * therefore marked, re-checked on every mutation inside a card (the GUI may
		 * re-render its header), and removed when this plugin is disposed.
		 */
		const CARD_TOGGLE_MARK = 'data-dsh-card-toggle'

		const cardToggleColor = 'var(--dsw-alias-label-secondary, inherit)'
		const cardToggleBorder = 'var(--dsw-alias-border-l2, rgba(128,128,128,0.4))'

		/** The header slot the toggle shares with the card's own icon buttons. */
		function cardToggleHost(card) {
			const header = card.querySelector('header')
			if (header === null) return null
			const iconButton = header.querySelector('button')
			if (iconButton !== null && iconButton.parentElement !== null) return iconButton.parentElement
			return header
		}

		/** Make sure the card carries the toggle, and that it reports the card's direction. */
		function syncCardToggle(card) {
			let toggle = card.querySelector('[' + CARD_TOGGLE_MARK + ']')
			if (toggle === null) {
				const host = cardToggleHost(card)
				if (host === null) return
				toggle = document.createElement('button')
				toggle.setAttribute('type', 'button')
				toggle.setAttribute(CARD_TOGGLE_MARK, '')
				toggle.style.padding = '0 6px'
				toggle.style.height = '24px'
				toggle.style.font = 'inherit'
				toggle.style.fontSize = '11px'
				toggle.style.lineHeight = '22px'
				toggle.style.borderRadius = '6px'
				toggle.style.cursor = 'pointer'
				toggle.style.border = '1px solid ' + cardToggleBorder
				toggle.style.background = 'transparent'
				toggle.style.color = cardToggleColor
				toggle.addEventListener('click', () => toggleCardDir(toggle.closest(QUERY_SEL)))
				host.appendChild(toggle)
			}
			// The label is the direction the card has *now*; the tooltip says what a
			// click would do with it. Reading it from the computed style is what keeps
			// the button honest in every case: the mode's rule, the Auto pass's `dir`
			// attribute, or the override this button itself wrote.
			const now = getComputedStyle(card).direction === 'rtl' ? 'RTL' : 'LTR'
			const next = now === 'RTL' ? 'LTR' : 'RTL'
			if (toggle.textContent !== now) toggle.textContent = now
			const title = 'Switch the question card to ' + next
			if (toggle.title !== title) toggle.title = title
			toggle.setAttribute('aria-label', title)
		}

		/** Every card on screen — the GUI mounts one, but nothing promises one. */
		function syncCardToggles() {
			if (styleEl === null) return
			for (const card of document.querySelectorAll(QUERY_SEL)) syncCardToggle(card)
		}

		/** Swap the card's own direction for the other one. */
		function toggleCardDir(card) {
			if (card === null) return
			cardDir = getComputedStyle(card).direction === 'rtl' ? 'ltr' : 'rtl'
			if (styleEl !== null) styleEl.textContent = cssFor(current)
			syncAutoPass()
			syncCardToggles()
		}

		/** Take every injected button back out of the GUI's card. */
		function removeCardToggles() {
			for (const toggle of document.querySelectorAll('[' + CARD_TOGGLE_MARK + ']')) {
				if (toggle.parentNode !== null) toggle.parentNode.removeChild(toggle)
			}
		}

		/**
		 * Cheap pre-filter for the shared observer: only a mutation inside a card, or
		 * one that adds a card, can change what a card's toggle has to say.
		 */
		function touchesCard(records) {
			for (const record of records) {
				const target = record.target
				if (target.nodeType === 1) {
					if (target.closest(QUERY_SEL) !== null) return true
				...
				for (const added of record.addedNodes) {
					if (added.nodeType !== 1) continue
					if (added.closest(QUERY_SEL) !== null || added.querySelector(QUERY_SEL) !== null) return true
				}
			}
			return false
		}
		...
		function apply(ctx) {
			...
			// The plugin's always-on observer. Every pass only ever writes a marker it
			// owns, so a table whose content changes under it is re-read, a table that
			// leaves the governed scopes (mode switch) gets its `dir` handed back, and
			// a card that the GUI mounts — or re-renders — gets its toggle.
			const observer = new MutationObserver((records) => {
				if (touchesTable(records)) scheduleTableSync()
				if (touchesCard(records)) syncCardToggles()
			})
			if (document.body !== null) {
			...
			syncAutoPass()
			// A question may already be on screen when the plugin is applied.
			syncCardToggles()
			...
			ctx.effect(() => () => {
				...
				// attributes; both go away with the fiber, and the buttons this plugin
				// put into the GUI's cards come back out with them.
				stopAutoPass()
				removeCardToggles()
