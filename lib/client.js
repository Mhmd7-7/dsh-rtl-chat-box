window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * - **Only elements that carry text.** Inside the card an element is marked
		 *   when it is the answer field, or a block a text run may live in, or an
		 *   element with a text node of its own. The card's flex rows (the option
		 *   row, the custom-answer row) hold no text of their own, so a marked row
		 *   can never mirror the layout.
		...
		/** The one card the GUI renders for one `ask_user_question` request. */
		...
		/**
		 * Hand an element back. Removing `dir` is guarded by the value this pass
		 * wrote, so an attribute some other code put there in the meantime survives.
		 */
		function clearAutoDir(el) {
			if (!el.hasAttribute(AUTO_DIR_MARK)) return
		...
		function syncAutoDirs() {
			...
			// The question card: its answer field, the blocks its text may live in,
			// and every element that carries a text node of its own — the eyebrow,
			// the option labels and their descriptions.
			//
			// List items are carved out. A card list is drawn by the GUI's markdown
			// sheet with `outside` markers and a *physical* left padding, so an item
			// given a direction of its own moves its number to the right edge with
			// no gutter to sit in: measured, that pushes the card body 6px past its
			// content box and, because the body scrolls, a horizontal scrollbar with
			// it. An Arabic list inside a question therefore keeps the layout the GUI
			// draws, while every other text in the card follows its own language.
		 ...
		 * question card re-renders on every answer — but only an added element that
		 * is, or sits in, or contains a list item or a question card can introduce
		 * an element this pass has not seen yet. A pure text update needs no pass:
		 * `dir="auto"` is re-derived by the engine when the content changes.
		 */
