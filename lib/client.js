window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function commit(next) {
			...
			// A mode change also changes which tables and which list items the
			// runtime passes govern.
			syncTableDirs()
			syncItemPass()
		}
		//#endregion
		...
		//#region auto list item direction
		/**
		 * ## Why Auto needs a runtime pass for `li` too
		 *
		 ...
		 *
		 * No CSS selector can read an element's text and set a property from it, so
		 * the per-item direction has to be written from script. `dir="auto"` is
		 * exactly that computation performed by the engine: it derives `direction`
		 * from the element's own first strong character, and re-derives it whenever
		 * the content changes — which is what finally moves the marker to the
		 * item's own side. It is an HTML attribute, not a style, so it is the only
		 * route to a per-item `direction`.
		 *
		 * The pass is deliberately narrow:
		 *
		 * - **Auto only.** RTL and LTR write one direction for the whole list in the
		 *   stylesheet, so nothing here may touch them.
		 * - **Only items with no `dir` of their own.** The composer is Lexical, and
		 *   Lexical writes `dir` on its blocks itself; an item that already carries
		 *   one is the editor's business, not the plugin's.
		 * - **Marked.** Every attribute this pass writes carries `ITEM_DIR_MARK`, so
		 *   leaving Auto hands back those and only those, never an editor's `dir`.
		 *
		 * It spans the same two surfaces the rest of Auto does: the transcript
		 * column and the composer host.
		 */
		const ITEM_DIR_MARK = 'data-dsh-item-dir'

		/** Every item the pass governs: the transcript column and the composer host. */
		const ITEM_SEL = both('li')

		/** True only in the one mode this pass exists for. */
		function itemScopeOn() {
			return current.mode === 'auto'
		}

		/** Write `dir` on an item that has none of its own, and mark it as ours. */
		function setItemDir(item) {
			if (item.hasAttribute('dir')) return
			item.setAttribute(ITEM_DIR_MARK, 'auto')
			item.setAttribute('dir', 'auto')
		}

		/**
		 * Hand an item back. Removing `dir` is guarded by the value this pass wrote,
		 * so an attribute some other code put there in the meantime survives.
		 */
		function clearItemDir(item) {
			if (!item.hasAttribute(ITEM_DIR_MARK)) return
			item.removeAttribute(ITEM_DIR_MARK)
			if (item.getAttribute('dir') === 'auto') item.removeAttribute('dir')
		}

		/** One pass over every item currently under this plugin's direction. */
		function syncItemDirs() {
			if (!itemScopeOn()) return
			for (const item of document.querySelectorAll(ITEM_SEL)) setItemDir(item)
		}
		 ...
		 * every token, and Lexical rewrites the composer on every keystroke, but
		 * only an added element that is, or contains, an `li` can introduce an item
		 * this pass has not seen yet.
		 */
		function touchesItem(records) {
			for (const record of records) {
				for (const added of record.addedNodes) {
					if (added.nodeType !== 1) continue
					if (added.tagName === 'LI' || added.querySelector('li') !== null) return true
				}
			}
			return false
		}
		...
		/** Streaming rewrites the transcript constantly; one pass per frame is plenty. */
		function scheduleItemSync() {
			if (itemFrame !== 0) return
			itemFrame = window.requestAnimationFrame(() => {
				itemFrame = 0
				syncItemDirs()
			})
		}

		/** Mark every item in scope and watch for the ones the app adds later. */
		function startItemPass() {
			if (itemObserver === null) {
				itemObserver = new MutationObserver((records) => {
					if (touchesItem(records)) scheduleItemSync()
				})
		...
		function stopItemPass() {
			if (itemObserver !== null) {
				itemObserver.disconnect()
				itemObserver = null
			}
			if (itemFrame !== 0) {
				window.cancelAnimationFrame(itemFrame)
				itemFrame = 0
			}
			for (const item of document.querySelectorAll('[' + ITEM_DIR_MARK + ']')) clearItemDir(item)
		}

		/** Follow the mode: the pass lives exactly as long as Auto is selected. */
		function syncItemPass() {
			if (itemScopeOn()) startItemPass()
			else stopItemPass()
		}
		//#endregion
		...
		function apply(ctx) {
			...
			syncTableDirs()
			// Auto may already be the stored preference, in which case the item pass
			...
			ctx.effect(() => () => {
				...
				// The item pass owns a second observer and a second set of `dir`
