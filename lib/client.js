window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 */
		const BODY_SEL = 'div, p, blockquote, dd, dt, td, th, h1, h2, h3, h4, h5, h6, code, table'
		...
		function cssFor(state) {
			...
			if (state.mode === 'auto' || state.smart) {
				...
				if (withText) rules.push(inChat(BODY_SEL) + ' { unicode-bidi: plaintext; }')د
				
				rules.push('[contenteditable]:not([contenteditable="false"]) :where(table) { direction: inherit; }')
				rules.push('[contenteditable]:not([contenteditable="false"]) :where(td, th) { unicode-bidi: plaintext; }')
			}
